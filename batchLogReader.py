...
def LogReader(filename,tier,movesets,ratings):
	...
	#doubles only
		...
		for line in log['log']:
			...
			if parsed_line[1] == 'switch' and parsed_line[2].startswith('p'):
				if len(parsed_line) < 4:
					sys.stderr.write('Problem with '+filename+'\n')
					sys.stderr.write('Could not parse line:\n')
					sys.stderr.write(line + '\n')
				...
				for s in aliases: #combine appearance-only variations and weird PS quirks
					if species in aliases[s]:
						species = s
						break
				username = ts[0][0] if parsed_line[2].startswith('p1') else ts[11][0]

				try:
					activeIndex=ts.index([username,species])
				except ValueError:
					#try undoing a mega evolution
					if species == 'Greninja-Ash':
						speciesBase = 'Greninja'
					...
					startIndex = 0 if parsed_line[2].startswith('p1') else 6
					for i in range(startIndex, startIndex+6):
						if ts[i][1].startswith(speciesBase):
							species = ts[i][1]
							activeIndex = i
					if activeIndex==-1:
		...
		if ("p3" not in log):
			for line in log['log'][start:]:
				...
				if linetype == "turn":
					...
					turnsOut[active[0]]=turnsOut[active[0]]+1
					turnsOut[active[1]]=turnsOut[active[1]]+1
					turnsOut[active[2]]=turnsOut[active[2]]+1
					turnsOut[active[3]]=turnsOut[active[3]]+1
					...
					if [ts[11*(int(line[p])-1)][0],species] not in ts:
						if species == 'Shaymin' and [ts[11*(int(line[p])-1)][0],'Shaymin-Sky'] in ts:
							...
							if not found:
								...
								sys.stderr.write(str([ts[11*(int(line[p])-1)][0],species])+'\n')
								return False
					if line[p:p+2] == '1a': activeIndex = 0
					if line[p:p+2] == '2a': activeIndex = 1
					if line[p:p+2] == '1b': activeIndex = 2
					if line[p:p+2] == '2b': activeIndex = 3
					active[activeIndex]=ts.index([ts[11*(int(line[p])-1)][0],species])
